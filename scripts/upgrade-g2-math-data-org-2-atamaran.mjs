...
await withDb(async (client) => {
  ...
  ) ?? matches.find((r) => !r.lesson_title?.includes("أفهم") && !r.lesson_title?.includes("أطبق") && !r.lesson_title?.includes("أراجع"));
  ...
  console.log("\nNew content score:", dryQuality.score, "| Issues:", dryQuality.issues.join(", ") || "none ✓");

  if (dryQuality.score < 85) {
    console.error(`❌ Quality gate failed: ${dryQuality.score}/100`);
    process.exit(1);
  }
  console.log(`✅ Quality gate passed: ${dryQuality.score}/100`);

  if (!apply) {
    console.log("\nDry run. No changes written. Lesson ID:", lesson.lesson_id);
    console.log("To apply: node scripts/upgrade-g2-math-data-org-2-atamaran.mjs --apply");
    return;
  }
  ...
  console.log("Post-apply score:", verifiedQ.score);
